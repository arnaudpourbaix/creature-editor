...
public final class IdsMap {
		...
		if (entry.getExtension().equalsIgnoreCase("IDS")) {
			while (st.hasMoreTokens()) {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "Error interpreting " + entry + ": " + token, "Error", JOptionPane.ERROR_MESSAGE);
					e.printStackTrace();
				}
			}
			if (entry.toString().equalsIgnoreCase("TRIGGER.IDS") || entry.toString().equalsIgnoreCase("ACTION.IDS")
	...
	private void extractIDS(String line) {
		...
		String param = null;
		i = val.indexOf((int) '(');
		if ((entry.toString().equalsIgnoreCase("ACTION.IDS") || entry.toString().equalsIgnoreCase("TRIGGER.IDS")) && i != -1) {
