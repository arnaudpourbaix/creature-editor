...
public class ColorConvert {
	...
	public static BufferedImage createCompatibleImage(int width, int height, boolean hasTransparency) {
		return createCompatibleImage(width, height, hasTransparency ? Transparency.TRANSLUCENT : Transparency.OPAQUE);
	}
	 ...
	 *            Image height in pixels
	 * @param transparency
	 *            The transparency type (either one of OPAQUE, BITMASK or TRANSLUCENT)
	 * @return A new BufferedImage object with the specified properties.
	 */
	 ...
	 * 
	 * @param img
	 *            The image to convert into a BufferedImage object.
	 * @param hasTransparency
	 *            Indicates whether the converted image should support transparency. (Does nothing if the specified image object is already a BufferedImage
	 *            object.)
	...
	public static BufferedImage toBufferedImage(Image img, boolean hasTransparency) {
		if (img != null) {
			if (img instanceof BufferedImage) {
				...
				final BufferedImage image = createCompatibleImage(img.getWidth(null), img.getHeight(null), hasTransparency);
	...
	/**
	 * Calculates the nearest color available in the palette for the specified color value. Note: For better color matching results the palette is expected in
	 * HSL color model.
	 * 
	 * @param rgbColor
	 *            The source color value in XRGB format (X is ignored).
	 * @param hslPalette
	 *            A HSL palette with the available color entries. Use the method {@link #toHslPalette(int[], int[])} to convert a RGB palette into the HSL
	 *            format.
	...
	/**
	 * Converts an array of colors from the RGB color model into the HSL color model. This is needed if you want to use the method
	 * {@link #nearestColor(int, int[])}.
	 * 
	 * @param rgbPalette
	 *            The source RGB palette.
	 * @param hslPalette
	 *            An array to store the resulting HSL colors into.
	 ...
	 * 
	 * @param color
	 *            The color value in XRGB format (X is ignored).
	 ...
	 * 
	 * @param pixels
	 *            The pixel block of the image in ARGB format (alpha is ignored).
	 * @param desiredColors
	 *            The resulting number of colors after reduction (range 1..256).
	 * @param ignoreAlpha
	 *            If <code>false</code>, only visible color values (alpha > 0) will be counted.
	 ...
	 *            The resulting number of colors after reduction (range 1..256).
	 * @param palette
	 *            The array to write the resulting colors into.
	 * @param ignoreAlpha
	 *            If <code>false</code>, only visible color values (alpha > 0) will be counted.
	...
	public static boolean medianCut(int[] pixels, int desiredColors, int[] palette, boolean ignoreAlpha) {
		...
		if (desiredColors > 0 && desiredColors <= MAX_COLORS && palette.length >= desiredColors) {
			PriorityQueue<PixelBlock> blockQueue = new PriorityQueue<PixelBlock>(desiredColors, PixelBlock.PixelBlockComparator);
			...
			while (blockQueue.size() < desiredColors) {
				...
				Arrays.sort(longestBlock.getPixels(), longestBlock.offset(), longestBlock.offset() + longestBlock.size(),
	...
	private static class Pixel {
		...
		public Pixel(int color) {
			this.color = new byte[] { (byte) ((color >>> 16) & 0xff), (byte) ((color >>> 8) & 0xff), (byte) (color & 0xff) };
		}
