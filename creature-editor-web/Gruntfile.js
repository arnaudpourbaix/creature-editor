...
module.exports = function(grunt) {
	...
	var taskConfig = {
		...
		/**
		 * `recess` handles our LESS compilation and uglification automatically. Only our `main.less` file is included in compilation; all other files must be imported from this
		 * file.
		 */
		recess : {
			build : {
		...
		index : {
			...
			build : {
				dir : '<%= build_dir %>',
				src : [ '<%= vendor_files.js %>', '<%= build_dir %>/src/**/*.js', '<%= html2js.common.dest %>', '<%= html2js.app.dest %>', '<%= vendor_files.css %>',
						'<%= recess.build.dest %>' ]
			},
	 ...
	 */
	grunt.registerTask('build', [ 'clean', 'html2js', 'jshint', 'recess:build', 'copy:buildAppAssets', 'copy:buildVendorAssets', 'copy:buildAppJs', 'copy:buildVendorJs',
			'copy:buildVendorCss', 'index:build', 'karmaconfig', 'karma:continuous' ]);
