...
(function(window, angular, log4javascript) {'use strict';
	 ...
	 * 
	 * log4javascript has hierarchical loggers, implemented in the same way as log4j. 
	 * In summary, you specify a logger's parent logger by means of a dot between the parent logger name and the child logger name. 
	 * Therefore the logger `tim.app.security` inherits from `tim.app`, which in turn inherits from `tim` which, finally, inherits from the root logger.
	 * 
	 * What inheritance means for a logger is that in the absence of a threshold level set specifically on the logger it inherits its level from its parent; 
	 * also, a logger inherits all its parent's appenders (this is known as appender additivity in log4j. This behaviour can be enabled or disabled via setAdditivity()). 
	 * In the above example, if the root logger has a level of DEBUG and one appender, each of the loggers tim.app.security, tim.app and tim would inherit the root level's appender. 
	 * If, say, tim.app's threshold level was set to WARN, tim's effective level would remain at DEBUG (inherited from the root logger) while tim.app.security's effective level would be WARN, inherited from tim.app. 
	 * The important thing to note is that appenders accumulate down the logger hierarchy while levels are simply inherited from the nearest ancestor with a threshold level set.
	 * 
	 ...
	 * });
	 * </pre>
	 * In this example, loggers have a default level of `ALL`. There is a root appender set on `ERROR` to catch error messages.
	 * It is possible to shutdown a logger because it won't try to append a message.
	 * But it is not possible to put a lower level priority on logger because this appender is set on `ERROR` and thus, will take precedence over logger.
	 * 
	 ...
	 * </pre>
	 * 
	 * First, we define a constant `LOGGER_NAME` which is a concatenation of angular module name with provider name. 
	 * Then, we define `log` which is just a shortcut for `$anLogger.logger(LOGGER_NAME)`.
	 * `LOGGER_NAME` constant is pretty usefull if we need to throw exception. See {@link an-error.$anError $anError} for more information.
	 * 
	 */
	 ...
	 * @description
	 * Use `$anLoggerProvider` to change the default behavior of the {@link an-log4javascript.$anLogger $anLogger} service.
	 * # Example
	 * <pre>
	 * angular.module('main.loggers', []).config(function($anLoggerProvider) {
	 *    $anLoggerProvider.level('ALL');
	...
	.provider('$anLogger', function anLoggerProvider() {
		 ...
		 * @description Object containing default values for {@link an-log4javascript.$anLogger $anLogger}. The object has following properties:
		 * 
		 * - **level** - `{log4javascript.Level object}` - Default level used when parameter is omitted.<br>
		 * - **layout** - `{string}` - Default layout pattern. Currently, all appenders use this layout so it must be defined here.<br>
		 * - **useBrowserConsole** - `{boolean}` - Uses browser's console instead of log4javascript's logger. It can be used to debug something but should be off most of the times.<br>
		 * 
		...
		/**
		 * @description Returns `true` if it is a production distribution.
		 * Production distribution omits all appenders except AjaxAppender, resulting in a drastically reduced file size compared to the standard edition.
		 */
		 ...
		 * @description
		 * # $anLogger
		 * This service is a wrapper for log4javascript and contains everything needed to properly use loggers in Angular.
		 */
		this.$get = function() {
			 ...
			 * @param {string} logger Logger's name. 
			 * @param {string} appender Appender's name `(Appender, AjaxAppender, AlertAppender, PopUpAppender, InPageAppender, BrowserConsoleAppender)`
			 * @param {Object} level log4javascript.Level object. 
			 * 
			 */
			 ...
			 * @param {string=} logger Logger's name. If omitted, `rootLogger` will be used.
			 */
			 ...
			 * - activate quiet mode
			 * - define root logger's level
			 * - add missing console levels (used when useBrowserConsole is true)
			 */
			(function init() {
