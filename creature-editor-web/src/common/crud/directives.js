...
.directive('crudAddButton', function() { 'use strict';
	return {
		restrict : 'E',
		replace : true,
...
.directive('crudButtons', function() { 'use strict';
	return {
		restrict : 'E',
		replace : true,
...
.directive('crudEdit', function($parse, $translate, toaster, $alertify) {  'use strict';
	...
	function sendNotification(key, type, notification, resource) {
		var message = $translate.instant(key, {
			entity: $translate.instant(notification.entity),
	...
	return {
		...
		link : function(scope, element, attrs, form) {
			...
			var onSaveError = function(result) {
				if (notification) {
					sendNotification('CRUD.SAVE_ERROR', 'error', params, resource);
				}
				userOnSaveError();
			};
			...
			var onRemoveError = function(result) {
				if (notification) {
					sendNotification('CRUD.REMOVE_ERROR', 'error', params, resource);
				}
