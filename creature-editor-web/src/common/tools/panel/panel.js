...
.factory('sidePanel', function ($q, $http, $templateCache, $injector, $controller, $compile, $rootScope, $document) {
	var defaults = {
		backdrop: true,
		keyboard: true
	};
	...
	function open(instance, settings) {
		console.log('open panel', settings);
		var body = $document.find('body').eq(0);
			
		var angularDomEl = angular.element('<div panel-window></div>');
		angularDomEl.attr({
			'window-class': settings.windowClass,
			'animate': 'animate'
		}).html(settings.content);
		var modalDomEl = $compile(angularDomEl)(settings.scope);
		body.append(modalDomEl);
		body.addClass(OPENED_MODAL_CLASS);
	}
